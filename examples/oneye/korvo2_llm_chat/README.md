...
### 7.1.16 ✅ 补契约 §2 的「限流 + 幂等」（帧面）＋一条吞吐观察（2026-09-17 续）

**背景**：契约 [C4] §2 有两条硬要求 —— **限流 ≤20 帧/s/对端、超限回 `error{code:"busy"}`** 与
**幂等（按帧 `id` 去重，窗口 60 s，重复 `id` 返回最后一次结果）**。巡检发现**两侧都没实现**
（例程 `lan_link.c` 与 SDK 均无相应逻辑）。本次在**数据面（例程）**补齐 —— 属承载层保护，不改 SDK 帧语义：

- **限流**：按对端 IPv4 做 1 s 滑窗计数（4 个槽位、满了轮转最旧；取不到对端 IP 则不限制），
  超限直接回 `busy` 且**不进入帧处理**；
- **幂等**：保留最近 4 条 `(id → 应答)`（TTL 60 s、单条应答 ≤1 KB，堆上存；内存不足则退化为不缓存）。
  命中时回上次结果并 `ESP_LOGI("幂等命中…不重复执行")`，**不重复执行副作用**。

**真机取证（新增探测开关 `--rate-probe` / `--idem-probe`）**：

| 断言 | 结果 | 证据 |
| --- | --- | --- |
| `LAN_E2E`（回归） | ✅ PASS | 发现 + `link.ping→link.pong`（nonce 一致）+ 无令牌守卫 |
| `LAN_IDEMPOTENT` | ✅ **PASS** | 同 `id` 两次应答**逐字一致**；设备侧日志 `幂等命中：id=pc-lan-e2e-idem-…（返回上次结果，不重复执行）` |
| `LAN_RATE_LIMIT` | ⚠️ **SKIP（不可触发）** | 连发 30 帧耗时 3.33 s ⇒ **实测 ≈9 帧/s < 阈值 20 帧/s**，本轮根本没超限 |

**为什么限流"测不了"（重要，勿误读为未实现）**：设备侧帧处理约 **110 ms/帧**（连 keep-alive 同连接
连发也只能到 ~9 帧/s），**低于 20 帧/s 这个阈值** ⇒ 串行流量永远触发不到限流分支。工具已改成**三态判定**
（出现 `busy` = PASS；未出现但吞吐 < 阈值 = SKIP 并说明；未出现且吞吐 ≥ 阈值 = FAIL），
避免把"触发不到"误报成"没实现"——本工具第一版正是用 urllib 每次新建连接（更慢）而误报 FAIL。

**待办（登记）**：① 要真正验限流，需并发连接把吞吐压到阈值以上，或把口径改成**按并发连接数**限流
（在契约评审里定）；② ~~设备侧 ~110 ms/帧 值得单独测一轮~~ **已测，结论＝非设备应用层缺陷**（见下）；
③ 带令牌的帧面路径仍未覆盖（令牌由 BLE 配对协商）。

**「≈110 ms/帧」的来源已定位（2026-09-17，零改动对照实验）**：先排除"客户端 Nagle/延迟 ACK"——
新连接逐条 203/109/110/109 ms、keep-alive 140/110/109/94/94/125 ms、**keep-alive + `TCP_NODELAY`
78/125/94/93/110/109 ms**（三组无差异 ⇒ 与 Nagle 无关）；再排除"设备应用层慢"——
同一 httpd 上**只回 404（不进我们的处理器）的 `GET /api/status` 也要 94–172 ms**，而**纯 TCP 连接建立
本身**（无 HTTP）已见 31/0/79 ms。⇒ **这 ~100 ms 是链路层台阶**（WiFi 适配器/AP 节能一类效应），
不是 `link.frame` 处理路径的开销；因此无需为此改固件。**对口径的影响**：PC（WiFi）侧串行吞吐
天花板 ≈9 帧/s，20 帧/s 的限流阈值只对"并发/流水线突发"才有意义。
