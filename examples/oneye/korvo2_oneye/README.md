...
**画面质量排查（第十九轮真机）**：面板侧反馈"花屏严重"后做了逐帧取证 —— 把设备发出的 MJPEG 全量存盘并按 multipart 切帧核对：

| 检查 | 结果 |
| --- | --- |
| 帧完整性（64 帧 @40 MHz / 62 帧 @20 MHz 两次采样） | **SOI 缺失 0 · EOI 缺失 0 · 后随边界异常 0** ⇒ **设备发出的每一帧都是完整合法 JPEG**（`output/.build/mjpeg-integrity.py`） |
| 帧内容（逐行亮度 + 彩噪指标 + ASCII 视图） | 行间跳变 0–1、高饱和像素 ~1.9%、彩噪指标 ~0.1% ⇒ 无撕裂、无彩噪（`cam-ascii.py` / `cam-analyze.py`） |
| 帧率 | 40 MHz ≈ **8.0 帧/s**、20 MHz ≈ **7.7 帧/s**（编码耗时主导，故两者接近） |
| multipart 分段字节 | 按 RFC 2046 写规范：`--boundary CRLF headers CRLF CRLF <jpeg> CRLF`（真机核对后去掉了多余的空行） |
| 会话结束原因 | 新增 `panel.camera.stream_ends` / `stream_end_reason`（如 `send-error (send=ESP_ERR_HTTPD_RESP_SEND, 40 帧/5097 ms)`）⇒ **"画面停在残帧上"这类问题从此可判**（页面上的"预览中"字样是启动时写死的，不能作准） |

⇒ 因此**若浏览器里仍见花屏**，问题在**客户端渲染/长连接中断**这一侧，而不是设备取帧：面板已按设备侧帧计数做**看护**（连续 3 次轮询无新帧 ⇒ 自动重连，连续 2 次失败 ⇒ 自动切轮询），并新增**「预览（轮询）」**按钮 —— 单帧轮询每次都拿完整 JPEG，**画面必定完整**（≈1 帧/s），可作为对照与兜底。
